...
function initAdf4jDemo() {
  ...
  const diagnosticsEl = root.querySelector<HTMLElement>(
    "[data-adf4j-diagnostics]",
  )!;
  const previewPanel = root.querySelector<HTMLElement>(
    '[data-adf4j-panel="preview"]',
  )!;
  const markdownPanel = root.querySelector<HTMLElement>(
    '[data-adf4j-panel="markdown"]',
  )!;
  const rawCode = root.querySelector<HTMLElement>("[data-adf4j-raw]")!;
  const previewTab = root.querySelector<HTMLButtonElement>(
    '[data-adf4j-view="preview"]',
  )!;
  const markdownTab = root.querySelector<HTMLButtonElement>(
    '[data-adf4j-view="markdown"]',
  )!;
  ...
  root
    .querySelector("[data-adf4j-toolbar]")
